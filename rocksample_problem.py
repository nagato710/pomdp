...
class ObservationModel(pomdp_py.ObservationModel):
    ...
    def sample(self, next_state, action, argmax=False):
        ...
        print(self._probs[next_state][action][observation])
...
class PolicyModel(pomdp_py.RolloutPolicy):
    ...
    #     print(random.sample(self.get_all_actions(state=state), 1)[0])
    ...
    def get_all_actions(self, **kwargs):
        # print("action is ",type(self._all_actions))
...
class SampleProblem(pomdp_py.POMDP):
    ...
    def generate_instance(n, k):
        ...
        for i in range(k):
            loc = SampleProblem.random_free_location(n, set(rock_locs.keys()) | set({rover_position}))
            rock_locs[loc] = i
    ...
    def __init__(self, n, k, init_state, rock_locs, init_belief):
        ...
        agent = pomdp_py.Agent(init_belief,
                               PolicyModel(n,k),
                               TransitionModel(n, rock_locs, self.in_exit_area),
                               ObservationModel(rock_locs),
                               RewardModel(rock_locs, self.in_exit_area))
...
def test_planner(rocksample, planner, nsteps=3, discount=0.95):
    ...
    for i in range(nsteps):
        ...
        TreeDebugger(rocksample.agent.tree).pp
...
def main():
    n, k = 5, 5
    init_state, rock_locs = SampleProblem.generate_instance(n, k)
    ...
    rocksample = SampleProblem(n, k, init_state, rock_locs, init_belief)
    # rocksample.print_state()
    print(rocksample.agent.policy_model)
    print("*** Testing POMCP ***")
