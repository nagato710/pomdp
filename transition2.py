...
def change_state(Xtest,rotate_list = None,data_type = 1,index = 0):
    ...
    if data_type == 1:
        ...
        for i in range(5):
            ...
            for j in range(4):
                if j < 3:
                    Obj_2.append(Xtest[index][i*3 + j + 6]*10)
                else:
        ...
        return push,pomdp_model2.State(Obj,Target,5)  
    ...
    if data_type == 2:
        ...
        return pomdp_model2.State(Obj,Target,5)  
...
if __name__ == '__main__':
    ...
    Obj = []
    Target = []
    # Target = pomdp_model2.set_Obj(Obj)

    # s1 = pomdp_model2.State(Obj,Target,5)
   
    f = open('testdata_new_2cm_action_normalized3.csv','r')
    # f = open('pos_data3.csv','r')
    reader = csv.reader(f)
    Xdata = []
    Ydata = []
    ...
    action1 = pomdp_model2.Action('a24')
    action2 = pomdp_model2.Action('a51')
    action3 = pomdp_model2.Action('a38')
    action4 = pomdp_model2.Action('a16')
    action5 = pomdp_model2.Action('a20')
    
    sample = pomdp_model2.transitionmodel(5)

    s2 = sample.sample(s1,action1)
    s2_ob = pomdp_model2.Observation(s2)
    ...
    s4 = sample.sample(s1,action1)
    s1.visualization(action1.drag,action1.push,name="s1+a8")
    s4.visualization(name = 's4')

    s5 = sample.sample(s4,action2)
    s4.visualization(action2.drag,action2.push,name="s4+a8")
    s5.visualization(name = 's5')

    s6 = sample.sample(s5,action3)
    s5.visualization(action3.drag,action3.push,name="s5+a8")
    s6.visualization(name = 's6')

    s7 = sample.sample(s6,action4)
    s6.visualization(action4.drag,action4.push,name="s6+a8")
    s7.visualization(name = 's7')

    s8 = sample.sample(s7,action5)
    s7.visualization(action5.drag,action5.push,name="s7+a8")
    s8.visualization(name = 's8')

    

    # s4_ob = pomdp_model2.Observation(s4)
    ...
    # s5_ob = pomdp_model2.Observation(s5)
    ...
    # s6_ob = pomdp_model2.Observation(s6)
